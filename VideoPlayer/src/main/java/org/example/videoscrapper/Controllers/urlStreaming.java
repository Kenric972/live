...
public void playVideo(String videoUrl) {
    ...
    // Update slider and label while playing
    mediaPlayer.currentTimeProperty().addListener((obs, oldTime, newTime) -> {
        if (!progressSlider.isValueChanging()) { // avoid conflict while dragging
            progressSlider.setValue(newTime.toSeconds());
        }
        timeLabel.setText(formatTime(newTime));
    });

// Set slider max when media is ready
    mediaPlayer.setOnReady(() -> {
...
// Seek when user clicks on the slider track
